...
func DevicesList(ctx context.Context, page, perPages int, filters map[string]string, sortField, sortOrder string) ([]devmod.Device, gmod.Pagination, int, int, error) {
	...
	for _, d := range rawResults {
		devices = append(devices, devmod.Device{
			...
			State:          d.State,
			Roi:            d.Roi,
			DateTimeCreate: utils.FormatTimeOrEmpty(d.DateTimeCreate),
			DateTimeUpdate: utils.FormatTimeOrEmpty(d.DateTimeUpdate),
		})
	}
...
func DeviceGetByID(ctx context.Context, id string) (*devmod.Device, error) {
	...
	dev := &devmod.Device{
		...
		State:          raw.State,
		Roi:            raw.Roi,
		DateTimeCreate: utils.FormatTimeOrEmpty(raw.DateTimeCreate),
		DateTimeUpdate: utils.FormatTimeOrEmpty(raw.DateTimeUpdate),
	}
